...
#
# Two-tier lookup:
#   1. _OUI_DB   — full IEEE MA-L database loaded from data/oui.json
#                  (~30 K entries, generated by bin/update-oui-db.py)
#   2. _BUILTIN_OUI — compact hand-curated fallback used when oui.json
#                  doesn't exist (e.g. offline first boot before install.sh
#                  ran the download, or unit tests).
#
# _OUI_DB is loaded lazily on the first call to get_mac_vendor() so that
# importing network_utils never blocks on I/O.

_OUI_DB: dict = {}
_OUI_DB_LOADED = False

# Candidate paths for the JSON database, checked in order.
_OUI_JSON_CANDIDATES = [
    Path(__file__).resolve().parent.parent / "data" / "oui.json",  # installed
    Path("/opt/risk-scanner/data/oui.json"),                        # explicit
]


def _load_oui_db() -> None:
    """Load the IEEE OUI JSON into _OUI_DB (called once, lazily)."""
    global _OUI_DB, _OUI_DB_LOADED
    _OUI_DB_LOADED = True  # set early so we don't retry on failure
    for path in _OUI_JSON_CANDIDATES:
        if path.exists():
            try:
                with open(path) as fh:
                    _OUI_DB = json.load(fh)
                logger.debug(f"OUI database loaded: {len(_OUI_DB):,} entries from {path}")
                return
            except Exception as exc:
                logger.warning(f"Could not load OUI database from {path}: {exc}")
    logger.debug("oui.json not found — using built-in OUI fallback table. "
                 "Run bin/update-oui-db.py to download the full IEEE database.")
...
def get_mac_vendor(mac: str) -> str:
    """
    Look up MAC address vendor.  Checks (in order):
      1. Full IEEE OUI database loaded from data/oui.json (~30 K entries)
      2. Built-in fallback table (common vendors, always available)
    Returns vendor name or 'Unknown'.
    """
    if not mac:
        return "Unknown"

    # Lazy-load the full OUI JSON on first call
    if not _OUI_DB_LOADED:
        _load_oui_db()

    mac_norm = normalize_mac(mac)
    oui = mac_norm[:8]  # "XX:XX:XX"

    # 1. Full IEEE database (exact 3-octet match)
    vendor = _OUI_DB.get(oui)
    if vendor:
        return vendor

    # 2. Built-in table (exact 3-octet match)
    vendor = _BUILTIN_OUI.get(oui)
    if vendor:
        return vendor

    # 3. Built-in 2-octet prefix fallback
    vendor = _BUILTIN_OUI_PREFIX2.get(mac_norm[:5])
    if vendor:
        return vendor
...
def get_subnets_from_interfaces() -> list:
    """
    Return list of CIDR subnet strings for all active non-loopback,
    non-virtual interfaces. Used to determine scan scope.
    Excludes docker/virbr/lo/veth interfaces.
    """
    # exclude virtual/docker/loopback names
    EXCLUDE_PREFIXES = ("lo", "docker", "virbr", "veth", "br-", "tun", "tap")
    ...
    for iface in interfaces:
        ...
        if cidr and cidr not in subnets:
            subnets.append(cidr)
    if subnets:
        logger.info(f"Scan subnets from interfaces: {', '.join(subnets)}")
    else:
        logger.warning("No scannable subnets found from network interfaces")
    return subnets


def resolve_credential_profile(ip: str, profiles: list) -> dict:
    """
    Return the best-matching credential profile for a given host IP.
    Matching priority (most-specific wins):
      1. Exact host match  (scope == 'host', target == ip)
      2. Subnet match      (scope == 'subnet', ip in subnet CIDR)
      3. Global default    (scope == 'global')
    Among ties at the same specificity level, lowest 'priority' value wins.
    ...
    for profile in profiles:
        ...
        if scope == "host":
            if ip in targets:
                host_matches.append((priority, profile))
        elif scope == "subnet":
            for cidr in targets:
                try:
                    if _ip.ip_address(ip) in _ip.ip_network(cidr, strict=False):
                        subnet_matches.append((priority, profile))
                        break
                except ValueError:
                    continue
        elif scope == "global":
            global_matches.append((priority, profile))

    for candidates in (host_matches, subnet_matches, global_matches):
        if candidates:
