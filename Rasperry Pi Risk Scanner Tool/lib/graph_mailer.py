...
graph_mailer.py - Microsoft Graph API Email Sender (importable module)
...
class GraphMailer:
    ...
    def _build_draft_message(
        ...
        """Build a draft message payload (no attachments — uploaded separately)."""
        to_recipients = [{"emailAddress": {"address": self.to_email}}]
        cc_recipients = []
        if cc_emails:
            cc_recipients = [{"emailAddress": {"address": addr}} for addr in cc_emails]
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if response.status_code == 202:
                    logger.info(f"Email sent successfully in {req_duration:.1f}s: '{subject}'")
                    return True
    ...
    def _send_with_upload_session(
        ...
        try:
            ...
            logger.info(f"Draft message created in {time.time() - draft_start:.1f}s")
    ...
    def _upload_large_attachment(self, draft_id: str, path: Path) -> None:
        ...
        logger.info(
            f"Attachment uploaded: '{path.name}' in {time.time() - upload_start:.1f}s"
        )
...
def load_mailer_from_config(
    config_path: str = "/opt/risk-scanner/config/config.json",
) -> GraphMailer:
    ...
    from_email = os.environ.get("GRAPH_FROM_EMAIL") or graph_cfg.get("from_email")
    to_email = os.environ.get("GRAPH_TO_EMAIL") or graph_cfg.get("to_email")
