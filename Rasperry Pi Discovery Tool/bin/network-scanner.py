...
Twenty-five-phase discovery:
  ...
  Phase 2b:  LLMNR / NBT-NS Passive Hostname Capture (runs concurrently with Phase 3)
  Phase 3:   Port Scanning (nmap top-N with -sV and --osscan-guess; RustScan pre-discovery)
  Phase 4:   Service Enumeration (HTTP, SMB/NetBIOS UDP-137/SMB2 impacket, SNMP, banners, NSE)
...
DEFAULT_CONFIG = {
    ...
    # ── LLMNR / NBT-NS passive hostname capture ────────────────────────────
    # Passively sniffs UDP 5355 (LLMNR) and UDP 137 (NBT-NS) for 60 seconds
    # concurrently with Phase 3.  Harvests Windows computer names with zero
    # active probing — catches machines that block TCP 445 but still broadcast
    # LLMNR/NetBIOS name queries.  Requires Scapy + cap_net_raw on python3.
    "enable_llmnr_capture": True,
    "llmnr_capture_duration": 60,
...
def _make_empty_host(ip: str, subnet_source: str = "direct") -> dict:
    return {
        ...
        "vendor": "Unknown",
        "hostname": None,
        "hostname_source": "",   # DNS | mDNS | SNMP | NetBIOS/SMB | NetBIOS-UDP137 | SMB2 | LLMNR | SSDP
        "logged_in_user": "",    # Windows logged-in username from NetBIOS type-0x03 (if discoverable)
        "open_ports": [],
        "services": {},
        "category": "Unknown",
        "security_flags": [],
...
# ── Phase 2b: LLMNR / NBT-NS Passive Hostname Capture ─────────────────────

def phase2b_llmnr_capture(config: dict, stop_event: threading.Event) -> dict:
    """Passively capture LLMNR (UDP 5355) and NBT-NS (UDP 137) traffic to
    harvest Windows computer names without any active probing.

    This phase is designed to run IN A BACKGROUND THREAD concurrently with
    Phase 3 (port scanning).  The active scanning traffic stimulates Windows
    hosts to send LLMNR / NBT-NS name queries, and their responses reveal the
    source computer's own hostname.

    How it works
    ────────────
    LLMNR (Link-Local Multicast Name Resolution, RFC 4795):
      Windows broadcasts name queries to 224.0.0.252:5355 (UDP).  When a host
      RESPONDS to a query, it sends from its own IP with sport=5355.  The
      question section of that response contains the name the host resolved —
      which is always the querier's own hostname in Windows autogenerated
      queries.  We capture source-IP → hostname from every QR=1 LLMNR packet.

    NBT-NS (NetBIOS Name Service, UDP 137):
      Same pattern: broadcast queries on 137; responses have sport=137.  The
      answer section contains the NetBIOS name being claimed, and the source IP
      is the claiming host.

    Returns
    ───────
    dict mapping IP → hostname (uppercase, no domain suffix).
    """
    duration = config.get("llmnr_capture_duration", 60)
    logger.info(
        f"[Phase 2b] LLMNR/NBT-NS passive capture started "
        f"(background, {duration}s)..."
    )

    captured: dict = {}   # ip -> hostname

    try:
        from scapy.all import sniff, IP, UDP, DNS
    except ImportError:
        logger.warning(
            "  [Phase 2b] Scapy not available — skipping LLMNR capture. "
    ...
    def _packet_handler(pkt):
        try:
            ...
            # ── LLMNR responses (source port 5355, QR=1) ──────────────────
            if sport == 5355 and DNS in pkt:
                dns = pkt[DNS]
                if dns.qr == 1 and dns.qdcount > 0:
                    # LLMNR query name = the hostname being resolved
                    try:
                        qname = dns.qd.qname.decode('utf-8', errors='ignore').rstrip('.').upper()
                        # Accept single-label names only (pure hostname, no domain)
                        if qname and '.' not in qname and len(qname) <= 15:
                            if src_ip not in captured:
                                logger.debug(f"  [Phase 2b] LLMNR: {src_ip} → {qname}")
                            captured[src_ip] = qname
                    except Exception:
                        pass

            # ── NBT-NS responses (source port 137) ────────────────────────
            elif sport == 137:
                payload = bytes(pkt[UDP].payload)
                if len(payload) < 12:
                    return
                # Parse NBT-NS packet header
                flags = struct.unpack('>H', payload[2:4])[0]
                qr = (flags >> 15) & 1
                if qr != 1:   # only process responses
                    return
                ancount = struct.unpack('>H', payload[6:8])[0]
                if ancount == 0:
                    return
                # Skip header (12) + question section
                # Question name: length-prefixed NBNS encoded (34 bytes) +
                #   type (2) + class (2) = 38 bytes
                ans_offset = 12 + 34 + 4   # = 50
                if len(payload) < ans_offset + 34 + 4 + 4 + 2:
                    return
                # Answer record: same NBNS name (34) + type(2)+class(2)+ttl(4)+rdlen(2)
                # = 44 bytes before RDATA; RDATA for NB record = 6 bytes per address
                # Decode the NBNS name in the answer (first 34 bytes at ans_offset)
                if len(payload) >= ans_offset + 16:
                    # NBNS name is 32 chars between two length bytes at ans_offset
                    encoded = payload[ans_offset + 1: ans_offset + 33]
                    if len(encoded) == 32:
                        decoded = ""
                        for j in range(0, 32, 2):
                            if encoded[j] >= ord('A') and encoded[j + 1] >= ord('A'):
                                hi = encoded[j] - ord('A')
                                lo = encoded[j + 1] - ord('A')
                                c = chr((hi << 4) | lo)
                                if c.isprintable():
                                    decoded += c
                        name = decoded.rstrip('\x00').rstrip(' ').strip()
                        if name and 2 <= len(name) <= 15:
                            if src_ip not in captured:
                                logger.debug(f"  [Phase 2b] NBT-NS: {src_ip} → {name}")
                            captured[src_ip] = name.upper()

        except Exception:
            pass   # never crash the capture thread

    try:
        # stop_function for scapy sniff: returns True when stop_event is set
        # or when duration has elapsed.
        t_end = time.time() + duration

        def _stop_filter(_pkt):
            return stop_event.is_set() or time.time() >= t_end
        ...
        logger.warning(f"  [Phase 2b] Capture error: {e}")

    logger.info(
        f"[Phase 2b] LLMNR/NBT-NS capture complete: "
        f"{len(captured)} hostname(s) harvested."
    )
    return captured
...
def _query_netbios_name_table(ip: str, timeout: float = 2.0) -> dict:
    """Query NetBIOS name table via UDP port 137 (Node Status Request / NBSTAT).

    Works on ALL Windows versions including 10/11 with SMBv1 disabled — the
    NetBIOS Node Status service (port 137 UDP) is independent of SMBv1/SMBv2.

    Returns a dict with keys:
      computer_name  — NetBIOS machine name (type 0x00, unique)
      domain         — Domain or workgroup name (type 0x00, group)
      logged_in_user — Currently logged-in username if present (type 0x03, unique)
      raw_names      — Full list of (name, type, flags) tuples

    Returns an empty dict on any failure (no exceptions raised to callers).
    """
    # Build NBSTAT query packet
    # Header: TxID(2) + Flags(2) + QDCOUNT(2) + ANCOUNT(2) + NSCOUNT(2) + ARCOUNT(2)
    txid = os.urandom(2)
    header = txid + b'\x00\x00' + b'\x00\x01' + b'\x00\x00' + b'\x00\x00' + b'\x00\x00'

    # QNAME: NBNS encoded wildcard "*" (all spaces → 0x00 bytes)
    # Each byte encoded as two nibbles + ord('A'):
    #   '*' (0x2A) → hi=2 → 'C' (0x43), lo=0xA → 'K' (0x4B)
    #   0x20 (padding) → hi=2 → 'C', lo=0 → 'A' → "CA"
    #   0x00 (null) → "AA"
    # The wildcard name is '*' + 15 null bytes (NBSTAT uses the null name)
    # '*' = 0x2A → "CK"; 0x00 → "AA" (×15)
    encoded_name = b'CK' + b'AA' * 15
    qname = bytes([32]) + encoded_name + b'\x00'       # 0x20 = 32 chars follow

    # QTYPE=0x0021 (NBSTAT), QCLASS=0x0001 (IN)
    question = qname + b'\x00\x21' + b'\x00\x01'
    ...
    # Parse response — minimum length sanity check
    if len(data) < 57:
        return result

    try:
        # Skip 12-byte header; answer name (same NBNS encoding = 34 bytes);
        # then type(2)+class(2)+ttl(4)+rdlength(2) = 10 bytes → data starts at byte 56
        # Number of names is the first byte of the RDATA
        name_count = data[56]
        offset = 57
        raw_names = []
        ...
        for _ in range(name_count):
            if offset + 18 > len(data):
                break
            # 15 bytes name (padded with spaces 0x20), 1 byte type, 2 bytes flags
            raw = data[offset:offset + 15].rstrip(b'\x00').rstrip(b' ').decode('ascii', errors='replace').strip()
            ntype = data[offset + 15]
            flags = struct.unpack('>H', data[offset + 16:offset + 18])[0]
            is_group = bool(flags & 0x8000)
            raw_names.append((raw, ntype, is_group))
            offset += 18

        result["raw_names"] = [(n, t, g) for n, t, g in raw_names]

        # Extract useful identities from the name table
        for name, ntype, is_group in raw_names:
            if not name:
                continue
            if ntype == 0x00 and not is_group:
                # Unique type-0x00 = computer name (workstation/server service)
                if "computer_name" not in result:
                    result["computer_name"] = name
            elif ntype == 0x00 and is_group:
                # Group type-0x00 = domain or workgroup
                if "domain" not in result:
                    result["domain"] = name
            elif ntype == 0x03 and not is_group:
                # Unique type-0x03 = currently logged-in user (Messenger service)
                # Filter out the machine account (same as computer_name)
                machine = result.get("computer_name", "")
                if name != machine:
                    result["logged_in_user"] = name
...
def _query_smb2_server_name(ip: str, timeout: int = 5) -> str | None:
    """Extract the Windows computer name from an SMB2 NEGOTIATE response.

    Sends a minimal unauthenticated SMB2 NEGOTIATE request to port 445 and
    reads the ServerName field from the NEGOTIATE_CONTEXT list in the response.
    This works on Windows 10/11 regardless of whether SMBv1 is enabled.

    Uses impacket if available; falls back to a raw SMB2 socket handshake.
    Returns the computer name string, or None on failure.
    """
    # ── Attempt 1: impacket SMBConnection (most reliable) ─────────────────
    try:
        from impacket.smbconnection import SMBConnection
        conn = SMBConnection('*SMBSERVER', ip, timeout=timeout)
    ...
    # ── Attempt 2: raw SMB2 NEGOTIATE (zero dependencies) ─────────────────
    # Build a minimal SMB2 NEGOTIATE request (dialects: 0x0202, 0x0210, 0x0300)
    # NetBIOS Session Service wrapper: type=0x00, length (3 bytes big-endian)
    # SMB2 Header (64 bytes) + NEGOTIATE body
    try:
        ...
        # NEGOTIATE request body: StructureSize(2)+DialectCount(2)+SecurityMode(2)+
        #   Reserved(2)+Capabilities(4)+ClientGuid(16)+NegotiateContextOffset(4)+
        #   NegotiateContextCount(2)+Reserved2(2)+ Dialects...
        dialects = b'\x02\x02\x10\x02\x00\x03'   # 0x0202, 0x0210, 0x0300
        neg_body = struct.pack('<HHHHI16sIHH',
            36,   # StructureSize
            3,    # DialectCount
        ...
        payload = smb2_header + neg_body
        nb_wrapper = b'\x00' + struct.pack('>I', len(payload))[1:]  # 3-byte length

        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.settimeout(timeout)
        sock.connect((ip, 445))
        sock.sendall(nb_wrapper + payload)

        # Read response — NetBIOS header (4) + SMB2 header (64) + NEGOTIATE body
        resp = b''
        sock.settimeout(timeout)
        try:
            while len(resp) < 200:
                ...
                resp += chunk
                if len(resp) >= 4:
                    msg_len = struct.unpack('>I', b'\x00' + resp[1:4])[0]
                    if len(resp) >= 4 + msg_len:
                        break
        except socket.timeout:
            pass
        sock.close()

        # Look for the ServerName string in the response (UTF-16-LE encoded)
        # ServerName appears in negotiate contexts after the NEGOTIATE_RESPONSE body.
        # Simple heuristic: scan for a plausible-length UTF-16-LE string of
        # printable ASCII characters after the fixed SMB2/NEGOTIATE structures.
        search_start = 68 + 65   # 4 NB + 64 SMB2 header + min negotiate body
        if len(resp) > search_start:
            # Parse utf-16-le runs; valid computer names are [A-Za-z0-9-_] ≤15 chars
            candidate = ""
            i = search_start
            while i + 1 < len(resp):
                lo, hi = resp[i], resp[i + 1]
                if hi == 0 and 0x20 <= lo <= 0x7e:
                    candidate += chr(lo)
                    i += 2
                else:
                    if 3 <= len(candidate) <= 15 and candidate.isidentifier():
...
def phase4_service_enumeration(hosts: list, config: dict) -> list:
    ...
    def enumerate_host(host: dict) -> dict:
        ...
        # SMB / NetBIOS hostname discovery
        # Three complementary techniques run in parallel:
        #   1. _query_smb2_server_name  — impacket SMB2 NEGOTIATE; works on
        #      Windows 10/11 regardless of SMBv1 status (most reliable)
        #   2. _query_netbios_name_table — UDP 137 NBSTAT; works even when
        #      TCP 445 is filtered; also yields domain + logged-in username
        #   3. _check_smb (nmap smb-os-discovery) — still used for OS string;
        #      less reliable on modern Windows but kept as OS-detail fallback
        if 445 in ports or 139 in ports:
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as _smb_ex:
                _f_smb2 = _smb_ex.submit(_query_smb2_server_name, host["ip"])
                _f_nb   = _smb_ex.submit(_query_netbios_name_table, host["ip"])
                _f_nmap = _smb_ex.submit(_check_smb, host["ip"])
                smb2_name  = _f_smb2.result(timeout=8)
                nb_info    = _f_nb.result(timeout=5)
                smb_info   = _f_nmap.result(timeout=18)

            # Merge into a single smb services dict.
            # Priority for computer_name: SMB2 > NetBIOS-UDP > nmap smb-os-discovery
            merged = dict(smb_info)   # start with nmap results (has OS string)
            if nb_info.get("computer_name"):
                merged["netbios_computer"] = nb_info["computer_name"]
            if nb_info.get("domain"):
                merged.setdefault("smb_domain", nb_info["domain"])
            if nb_info.get("logged_in_user"):
                merged["logged_in_user"] = nb_info["logged_in_user"]
            if smb2_name:
                merged["smb2_computer"] = smb2_name

            # Canonical smb_computer: prefer smb2 > netbios-UDP > nmap
            if smb2_name:
                merged["smb_computer"] = smb2_name
            elif nb_info.get("computer_name"):
                merged["smb_computer"] = nb_info["computer_name"]
            # (else: keep whatever nmap returned, which may be empty)
...
def _enrich_hostnames_from_discovery(
    ...
    mdns_results: dict,
    ssdp_results: dict,
    llmnr_results: dict | None = None,
) -> list:
    """
    Enrich host hostname fields using data already collected by later phases.

    Priority (highest → lowest):
      1. SMB2/impacket  — smb2_computer from unauthenticated SMB2 NEGOTIATE
                          (works on Windows 10/11 with SMBv1 disabled)
      2. NetBIOS-UDP137 — netbios_computer from direct UDP-137 NBSTAT query
      3. NetBIOS/nmap   — smb_computer from nmap smb-os-discovery
                          (legacy; may be empty on modern Windows)
      4. LLMNR          — hostname harvested from passive LLMNR/NBT-NS capture
      5. SNMP sysName   — authoritative device name set by admin
      6. mDNS / Bonjour — .local names from avahi-browse
      7. UPnP / SSDP    — friendly_name from UPnP description XML
    ...
    Also copies logged_in_user from SMB/NetBIOS data to the top-level host dict.
    """
    ...
    # LLMNR/NBT-NS results are already {ip: hostname}
    llmnr_by_ip: dict = llmnr_results or {}
    ...
    for host in hosts:
        ...
        # Always surface logged_in_user to the top-level host dict so reports
        # can display it regardless of whether hostname was already resolved.
        if smb.get("logged_in_user") and not host.get("logged_in_user"):
            host["logged_in_user"] = smb["logged_in_user"]
        ...
        # Priority chain for hostname assignment
        if smb.get("smb2_computer"):
            host["hostname"] = smb["smb2_computer"]
            host["hostname_source"] = "SMB2"
            named_new += 1
        elif smb.get("netbios_computer"):
            host["hostname"] = smb["netbios_computer"]
            host["hostname_source"] = "NetBIOS-UDP137"
            named_new += 1
        elif smb.get("smb_computer"):
            host["hostname"] = smb["smb_computer"]
            host["hostname_source"] = "NetBIOS/SMB"
            named_new += 1
        elif ip in llmnr_by_ip:
            host["hostname"] = llmnr_by_ip[ip]
            host["hostname_source"] = "LLMNR"
            named_new += 1
    ...
    if named_new:
        logger.info(
            ...
            f"from SMB2/NetBIOS/LLMNR/mDNS/SSDP/SNMP."
        )
    return hosts
...
def run_discovery(progress_callback=None) -> dict:
    ...
    # ── Phase 2b: LLMNR/NBT-NS passive capture (background thread) ───────
    # Start the passive capture NOW, before Phase 3 begins.  The active port
    # scanning traffic from Phase 3 stimulates Windows hosts to send LLMNR /
    # NBT-NS name queries whose responses reveal each host's own computer name.
    _llmnr_stop  = threading.Event()
    _llmnr_result_container: list = [{}]   # mutable container for thread result

    def _llmnr_thread_target():
        if config.get("enable_llmnr_capture", True):
            _llmnr_result_container[0] = phase2b_llmnr_capture(config, _llmnr_stop)
        else:
            logger.info("Phase 2b (LLMNR/NBT-NS capture): SKIPPED (disabled in config)")

    _llmnr_thread = threading.Thread(target=_llmnr_thread_target, daemon=True,
                                     name="llmnr-capture")
    _llmnr_thread.start()
    ...
    # Collect Phase 2b results (signal stop and wait for graceful shutdown)
    _llmnr_stop.set()
    _llmnr_thread.join(timeout=10)
    llmnr_results = _llmnr_result_container[0]
    if llmnr_results:
        logger.info(
            f"Phase 2b (LLMNR/NBT-NS capture): "
            f"{len(llmnr_results)} hostname(s) captured."
        )
    phase_timings.append({
        "phase": "2b", "name": "LLMNR/NBT-NS passive hostname capture",
        "duration": config.get("llmnr_capture_duration", 60),
    ...
    # Enrich host hostname fields from all discovery sources now that phases
    # 2b (LLMNR), 4 (SMB/NetBIOS), 8 (mDNS), and 9 (SSDP) have completed.
    hosts = _enrich_hostnames_from_discovery(
        hosts, mdns_results, ssdp_results, llmnr_results
    )
